<template>
  <div class="admin-curso">
      ...
      <form @submit.prevent="guardarCurso" class="curso-form">
        ...
        <div class="field checkbox">
          <label>
            ...
            Curso activo (visible en el catÃ¡logo)
          </label>
        </div>
        <div class="acciones">
    ...
    <details class="admin-card">
      <summary>Agregar mÃ³dulo</summary>
      <form @submit.prevent="agregarModulo" class="inline-form">
        <input v-model="nuevoModulo" type="text" placeholder="TÃ­tulo del mÃ³dulo (ej: Unidad 1)" required />
        <button type="submit" class="btn-primary" :disabled="cargando">Agregar</button>
      </form>
    </details>

    <div v-if="!curso.modulos?.length" class="empty">
      TodavÃ­a no hay mÃ³dulos. Agrega el primero con la opciÃ³n de arriba.
    </div>
    ...
    <div v-for="modulo in curso.modulos" :key="modulo.id" class="modulo-admin">
      <div class="modulo-admin-head">
        <template v-if="editandoModuloId === modulo.id">
          ...
          <button class="btn-min" @click="guardarModulo(modulo)">âœ“</button>
          <button class="btn-min btn-ghost" @click="cancelarEdicionModulo">âœ•</button>
        </template>
        <template v-else>
          <h3 class="modulo-titulo">ðŸ“š {{ modulo.titulo }}</h3>
        ...
        <li v-for="clase in modulo.clases" :key="clase.id" class="clase-admin-item">
          <template v-if="editandoClaseId === clase.id">
            <div class="clase-edit">
              <input v-model="formClaseEdit.titulo" type="text" class="edit-input" placeholder="TÃ­tulo" />
              ...
              <button class="btn-min" @click="guardarClase(clase, modulo)">âœ“</button>
              <button class="btn-min btn-ghost" @click="editandoClaseId = null">âœ•</button>
            </div>
          </template>
          <template v-else>
            <div class="clase-fila">
              ...
              <div class="clase-info">
                ...
                <span class="clase-meta">
                  ...
                  <template v-if="clase.duracion_minutos"> Â· {{ clase.duracion_minutos }} min</template>
                  <span v-if="clase.es_gratis" class="gratis"> Â· Gratis</span>
                </span>
              </div>
      ...
      <p v-else class="empty-clases">Sin clases todavÃ­a.</p>
      ...
      <form v-if="mostrarFormClase === modulo.id" @submit.prevent="agregarClase" class="clase-form">
        <input v-model="formNuevaClase.titulo" type="text" placeholder="TÃ­tulo de la clase" required />
        <input v-model="formNuevaClase.url_archivo" type="url" placeholder="https://..." />
        <select v-model="formNuevaClase.tipo_archivo">
          <option value="video">Video</option>
          <option value="pdf">PDF</option>
        </select>
        <input v-model.number="formNuevaClase.duracion_minutos" type="number" min="0" placeholder="DuraciÃ³n (min)" />
        <label class="chk">
...
function iconoClase(tipo) {
  // Elige el emoji segÃºn el tipo de archivo de la clase
  if (tipo === "video") return "ðŸŽ¬";
  if (tipo === "pdf") return "ðŸ“„";
  return "ðŸ“";
}

async function notificar(fn) {
  // Ejecuta una acciÃ³n de la API mostrando el error si falla
  error.value = "";
  mensajeOk.value = "";
  try {
    await fn();
  } catch (e) {
    error.value = e.response?.data?.error || "Error al ejecutar la acciÃ³n";
  }
}
...
async function eliminarCurso() {
  if (!window.confirm("Â¿Seguro que querÃ©s eliminar este curso?")) return;
  await notificar(async () => {
...
async function eliminarModulo(modulo) {
  if (!window.confirm(`Â¿Eliminar el mÃ³dulo "${modulo.titulo}" y todas sus clases?`)) return;
  await notificar(async () => {
...
async function eliminarClase(clase, modulo) {
  if (!window.confirm(`Â¿Eliminar la clase "${clase.titulo}"?`)) return;
  await notificar(async () => {
    await api.delete(
      `/cursos/${props.curso.id}/modulos/${modulo.id}/clases/${clase.id}`
    );
